...
if "%publish%"=="local" (
	%buildExe% WinSvcExecutor.csproj /p:Configuration=%config% /t:AfterBuild,Package,PublishLocal
)
:: build with nuget remote publish
if "%publish%"=="nuget" (
	%buildExe% WinSvcExecutor.csproj /p:Configuration=%config% /t:AfterBuild,Package,PublishNuGet
)
:: build without publish
if "%publish%"=="none" (
	%buildExe% WinSvcExecutor.csproj /p:Configuration=%config%
)
